...
Each item gets five scores from 1–7:
...
| Cost    | 1 = cheap, 7 = expensive        | higher = *later* |

**Cost runs backwards on purpose.** An expensive thing should be harder to
justify, so cost enters the ranking flipped (a 7 counts as a 1). It's the amber
slider, so you can see at a glance which factor is arguing against the purchase.

The list is sorted by a weighted average of the five, shown out of 7. The
...
- Lists saved on the earlier 1–10 scale are rescaled proportionally on load
